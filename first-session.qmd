...
You should see the `>` prompt --- this is Claude Code's prompt, not your terminal's. **From here on, almost everything you type goes into Claude Code, not your terminal.** Whenever a code block on this page starts with `>`, that's a prompt for Claude.
...
### 2. Clone the LINCS skills repository

```
Clone https://github.com/jburnford/lincs_skills.git into this folder
as a subfolder called lincs_skills/.
```

This pulls in the seven LINCS-specific skills (CIDOC-CRM, the LINCS application profile, place/person disambiguation, etc.) that we will use later in the workshop.
...
You only ever do this setup once. From now on, any time you reopen the workshop, you just `cd claude_workshop && claude` and pick up where you left off.
...
1. Ask Claude Code to count the arrivals by month across the whole period and produce a table.
2. Ask it to map the top 10 ports of departure to modern country names.
3. Ask it to filter to ships carrying timber and list the top 10 ports for the timber trade.
...
3. Ask it to produce a citation table with author, title, year, and format for all 10 items.
